                  ...
                  keep.diss = n < 100, keep.data = !diss, trace.lev = 0)
{
  ...
  meth <- pmatch(method, METHODS) ### returns position of method in METHODS
  if(is.na(meth)) stop("invalid clustering method")
  if(meth == -1) stop("ambiguous clustering method")
  cl. <- match.call() ### stores the specified arguments in agnes
  method <- METHODS[meth] ### store the name of the method
  # if(method == "flexible") {
  ...
  if((diss <- as.logical(diss))) { ### TRUE if x has class "dist"
    ...
    if(data.class(x) != "dissimilarity") { # try to convert to
      if(!is.null(dim(x))) {
        ...
        if(!is.numeric(x) || is.na(n <- sizeDiss(x))) ### sizeDist checks that length(d)= n(n-1)/2, returns length(d) if TRUE
          stop("'x' is not and cannot be converted to class \"dissimilarity\"")
        attr(x, "Size") <- n # nrow x
      }
      class(x) <- dissiCl
      if(is.null(attr(x,"Metric"))) attr(x, "Metric") <- "unspecified"
    }
    n <- attr(x, "Size")
    dv <- x[lower.to.upper.tri.inds(n)] ### creates vector of upper triangular entries
    ## prepare arguments for the Fortran call
    dv <- c(0., dv)# "double", 1st elem. "only for Fortran" (?)
    jp <- 1L
    mdata <- FALSE
    ndyst <- 0
    x2 <- double(1) ### vector type double length 1
  }
  else { ### if input is not a distance matrix
    ...
    #   x2[inax] <- valmisdat
    # }
    dv <- double(1 + (n * (n - 1))/2) ### creates double vector length of triangular number +1 (to match above)
  }
  ...
  res <- .C(twins,
            as.integer(n), ### number of samples
            as.integer(jp), ### number of variables (1 if diss TRUE)
            x2, ### data matrix (double(1) is dist matrix supplied)
            dv, ### upper triangular with leading zero if dist, vector of zeros if not dist
            dis = double(if(C.keep.diss) length(dv) else 1), ### in which to store dist matrix if necessary
            jdyss = if(C.keep.diss) diss + 10L else as.integer(diss),
            if(mdata) rep(valmisdat, jp) else double(1), ### 0 unless missing data
            if(mdata) jtmd else integer(jp), ### number of vars unless missing data
            as.integer(ndyst), ### 0 if distance matrix, 2 if manhatton, 1 otherwise
            1L,# jalg = 1 <==> AGNES
            meth,# integer
            integer(n),
            ner = integer(n),
